const APP_BUILD_ID = "v002-final-ui-homecard-hacs-20260517";
...
class RFLinkRawToolsPanel extends HTMLElement {
  ...
  _infoView() {
    ...
    return `<section class="addon-card info-card"><div class="addon-head"><div><h2>RFLink Raw Tools (Beta)</h2><p>Current version: ${this._escape(s.version || "0.0.2")}</p></div><div class="connection ${this._tone()}">${this._tone() === "ok" ? "●" : "⊘"}</div></div><div class="badges"><span class="badge green">RF 433 MHz</span><span class="badge blue">Home Assistant</span><span class="badge blue">RFLink</span><span class="badge green">Switches</span><span class="badge blue">Capture</span></div><p class="desc">A Home Assistant sidebar app for RFLink setup, RF capture, learned command sending, friendly aliases, and unsupported-remote evidence.</p><p class="desc"><b>Status:</b> ${this._escape(s.readiness_detail || "Loading status…")}</p><div class="settings-list"><div class="setting-row"><div><b>RFLink YAML</b><span>${s.rflink_configured ? "Configured. Configuration can install or remove the RFLink Raw Tools managed block." : "Not configured. Configuration can add a conservative top-level rflink: block."}</span></div>${this._miniState(s.rflink_configured ? "Configured" : "Missing", s.rflink_configured)}</div><div class="setting-row"><div><b>Home Assistant RFLink bridge</b><span>${s.rflink_loaded ? "Loaded" : "Not loaded"} / ${s.rflink_connected ? "Connected" : "Loaded"}.</span></div>${this._miniState(s.rflink_loaded ? "Loaded" : "Not loaded", s.rflink_loaded)}</div><div class="setting-row"><div><b>Alias-backed HA switches</b><span>Teach/Alias records with ON and OFF commands can appear as Home Assistant switches.</span></div>${this._miniState(o.alias_switches_enabled ? "Enabled" : "Disabled", o.alias_switches_enabled)}</div><div class="setting-row"><div><b>Sidebar app</b><span>Controls whether RFLink Raw Tools appears in the left sidebar.</span></div>${this._miniState(o.sidebar_enabled ? "Shown" : "Hidden", o.sidebar_enabled)}</div><div class="setting-row"><div><b>Overview dashboard card</b><span>Button card on the default Overview dashboard for opening RFLink Raw Tools.</span></div>${this._miniState(o.home_card_installed ? "Installed" : "Not installed", o.home_card_installed)}</div></div><div class="footer-actions"><button data-tablink="configuration">Configuration</button><button class="secondary" data-tool="send">Send</button><button class="secondary" data-tool="captured">Captured</button><button class="secondary" data-tool="teach">Teach</button><button class="secondary" data-tablink="log">Log</button></div></section><section class="addon-card tools-card"><h2>RFLink Tools</h2><div class="tool-tabs"><button class="tool ${this._state.tool === "send" ? "active" : ""}" data-tool="send">Send</button><button class="tool ${this._state.tool === "captured" ? "active" : ""}" data-tool="captured">Captured</button><button class="tool ${this._state.tool === "teach" ? "active" : ""}" data-tool="teach">Teach Alias</button></div>${this._toolView()}</section>`;
  }

  _configurationView() {
    const s = this._state.status || {}; const o = this._options();
    return `<section class="addon-card"><h2>Configuration</h2><p class="desc">Switches and maintenance actions for RFLink YAML, sidebar placement, Overview dashboard card, and alias-backed Home Assistant switches.</p><div class="config-grid"><div class="config-box"><h3>RFLink YAML</h3><p>${s.rflink_configured ? "RFLink is configured." : "RFLink is not configured."}</p><label>Serial port</label><input data-field="port" value="${this._attr(this._state.port)}"><div class="actions"><button data-action="_installYaml">Install / repair RFLink YAML</button><button class="danger" data-action="_removeYaml">Remove RFLink YAML</button></div><p class="note">Remove only deletes the block created by RFLink Raw Tools. It refuses to delete user-managed YAML.</p></div><div class="config-box"><h3>App placement</h3>${this._toggleRow("Show in left sidebar", "Turn off only if you plan to open /rflink-raw-tools directly.", "sidebar_enabled", o.sidebar_enabled)}<div class="setting-row"><div><b>Overview dashboard card</b><span>Adds or removes an RFLink Raw Tools button on the default Overview dashboard.</span></div><div class="inline-actions"><button class="secondary" data-action="_installHomeCard">Add to Overview</button><button class="secondary" data-action="_removeHomeCard">Remove from Overview</button></div></div></div><div class="config-box"><h3>Home Assistant entities</h3>${this._toggleRow("Expose aliases as HA switches", "Aliases with ON/OFF commands become optimistic Home Assistant switch entities.", "alias_switches_enabled", o.alias_switches_enabled)}<p class="note">After enabling or creating aliases, restart Home Assistant if entities do not appear immediately.</p></div><div class="config-box"><h3>Maintenance</h3><div class="actions"><button class="secondary" data-action="_checkSetup">Check setup</button><button class="secondary" data-action="_clearStatus">Clear status</button></div><p class="note">HACS/GitHub releases handle app updates. There is no self-updater in the integration.</p></div></div></section>`;
  }
