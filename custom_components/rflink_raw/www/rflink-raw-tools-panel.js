const APP_BUILD_ID = "v003-dashboard-shortcut-helper-20260517";
  ...
  _configurationView() {
    ...
    return `<section class="addon-card config-page">
      ...
      <div class="config-grid improved">
        ...
        <div class="config-box placement-box"><h3>App placement</h3><div class="option-line"><div><b>Show in left sidebar</b><span>Turn off only if you plan to open <code>/rflink-raw-tools</code> directly.</span></div><label class="switch"><input type="checkbox" data-option-toggle="sidebar_enabled" ${o.sidebar_enabled ? "checked" : ""}><span class="slider"></span></label></div><div class="overview-card-block"><div><b>Dashboard shortcut</b><span>Home Assistant automatic Overview dashboards are not safely writable by custom integrations. RFLink Raw Tools stays in the sidebar. To add a dashboard shortcut, copy this Manual card YAML and paste it into any dashboard.</span></div><textarea class="copy-yaml" readonly>${this._escape(this._dashboardCardYaml())}</textarea><div class="config-card-actions"><button class="secondary" data-action="_copyDashboardCard">Copy card YAML</button><button class="secondary" data-action="_openOverview">Open Overview</button><button class="secondary" data-action="_openApp">Open RFLink Raw Tools</button></div></div></div>
