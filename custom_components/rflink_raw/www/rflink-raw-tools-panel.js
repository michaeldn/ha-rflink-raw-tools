const APP_BUILD_ID = "v002-home-status-top-fix-20260517";
...
class RFLinkRawToolsPanel extends HTMLElement {
  ...
  _infoView() {
    ...
    return `<section class="addon-card info-card">
      ...
      <div class="settings-list">
        ...
        <div class="setting-row"><div><b>Home screen card</b><span>Button card on the default Overview dashboard for opening RFLink Raw Tools.</span></div>${this._miniState(o.home_card_installed ? "Installed" : "Not installed", o.home_card_installed)}</div>
      </div>
  ...
  _configurationView() {
    ...
    return `<section class="addon-card"><h2>Configuration</h2><p class="desc">These are the switches and maintenance actions for RFLink YAML, sidebar placement, Home screen card, and alias-backed Home Assistant switches.</p>
      <div class="config-grid">
        ...
        <div class="config-box"><h3>App placement</h3>${this._toggleRow("Show in left sidebar", "Turn off only if you plan to open /rflink-raw-tools directly.", "sidebar_enabled", o.sidebar_enabled)}<div class="setting-row"><div><b>Home screen card</b><span>Adds or removes an RFLink Raw Tools button on the default Overview dashboard.</span></div><div class="inline-actions"><button class="secondary" data-action="_installHomeCard">Add to Overview</button><button class="secondary" data-action="_removeHomeCard">Remove from Overview</button></div></div></div>
  ...
  _logView() {
    ...
    return `<section class="addon-card"><h2>Log</h2><p class="desc">Logging controls are here, separate from setup actions.</p>
      ...
      <div class="actions"><button data-action="_loadLogs">Refresh log lines</button><button class="secondary" data-action="_clearStatus">Clear status</button></div><div class="log-list">${rows || `<div class="empty">No RFLink log lines found yet.</div>`}</div></section>`;
  }
  ...
  _topStatusDetail() {
    ...
    const s = this._state.status || {};
    return s.last_error || s.last_result || s.readiness_detail || "";
  }

  _statusBadge() {
    const detail = this._topStatusDetail();
    return `<div class="status-cluster">
      <span class="status-pill ${this._tone()}">• ${this._escape(this._statusLabel())}</span>
      ${detail ? `<span class="status-detail">${this._escape(detail)}</span>` : ""}
      <button class="status-clear" data-header-action="_clearStatus">Clear</button>
    </div>`;
  }
