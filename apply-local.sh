...
ZIP="/Users/michaeldumas/Downloads/ha-rflink-raw-tools-v0.0.1-runtime-cache-check-fix-onefile.zip"
EXTRACT="/Users/michaeldumas/Downloads/ha-rflink-raw-tools-v0.0.1-runtime-cache-check-fix-extract"
...
mkdir -p "$BACKUP"; cp -R "$REPO" "$BACKUP/repo"
cd "$REPO"
...
for p in list(Path(".").rglob("*.pyc")): p.unlink()
if errors: raise SystemExit(errors)
if list(Path(".").rglob("__pycache__")) or list(Path(".").rglob("*.pyc")): raise SystemExit("Cache artifacts still present")
print("Python compile and no-cache checks passed.")
PY
git status --short
git add -A
git commit -m "Clarify RFLink runtime cache checks" || true
